...
class ArticleEdit extends Component{
	componentDidMount(){
	    ...
	    //publish
	    $('[data-tag=publish]').on('click',function(e){
	    	if(self.props.aId<=0){return;}
		...
		class Tag {
			...
			appendTag(val){
				...
				if(obj&&obj.tags.indexOf(id)>-1){return;}
				if(!obj){
					obj=Tags[val]={
						id:0,
						name:val,
	...
	componentWillReceiveProps(nextProps){
		...
		if(nextProps.is_publish==1){
			$('[data-tag=publish]').attr('class','fa fa-check no-disable').html(' 已经发布');
		} else {
			$('[data-tag=publish]').attr('class','fa fa-check no-disable').html(' 开始发布');
		}
	...
	render(){
		...
		return (
			...
			<div className="title-wrap">
				<input ref="title" className="title" type="text" name="title" id="title" onChange={this.changeTitle} value={title||'无标题文章'} placeholder="这是标题" />
			</div>
			<div className="tag-wrap">
				<i className="fa fa-tags"></i>
				<ul id="tagList">
					{
						tags.map((item,index)=> <li key={item.tag_id}>{item.name}&nbsp;
