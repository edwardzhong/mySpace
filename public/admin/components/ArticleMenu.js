...
class ArticleMenu extends Component{
	...
	render(){
		...
		return(
			<div id="articleMenu" className={arrowDown?'menu arrow-down':'menu'} style={{
			...
			article?
				...
				{ 
					...
					<li onClick={()=>this.setPublish(article.id,article.is_publish)} data-tag="publish" className="can-hover"><i className="fa fa-mail-forward"></i> 直接发布</li>
					:<li><i className="fa fa-check"></i> 已经发布</li>
				}
					<li data-tag="history" className="can-hover"><i className="fa fa-clock-o"></i> 历史版本</li>
					<li onClick={()=>this.setDelete(article.id,article.is_delete)} data-tag="delete" className="can-hover"><i className="fa fa-trash"></i> 删除文章</li>
				</ul>
				:<ul></ul>
			}
			</div>
