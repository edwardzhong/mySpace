...
const methods={
    ...
    getArticleByUserId:'select id,title,content,author_id,is_good,is_top,is_publish,is_delete,FROM_UNIXTIME(create_date, \'%Y/%m/%d %h:%m:%s\')create_date ,from_unixtime(update_date, \'%Y/%m/%d %h:%m:%s\')update_date from article where author_id=? order by id desc',
    ...
    setPublish:'update article set is_publish=? where id=?',
    setDelete:'update article set is_delete=? where id=?'
};
